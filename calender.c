...
// to disable it so that we have to press enter for any thing to register in
// terminal
void disableRawMode() {
  struct termios term;
  tcgetattr(STDIN_FILENO, &term);
  ...
  for (int row = 0; row < ROWS; row++) {
    ...
    for (int col = 0; col < COLS; col++) {
      if (date <= daysInMonth && (row > 0 || col >= dayOfWeek)) {
        ...
        if (date == day) {
          printf("\033[7m %2d \033[0m│",
                 date); // ansi character to inverse colors
